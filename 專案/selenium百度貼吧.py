...
SEARCH_HEADERS = {
    ...
    "referer": "https://tieba.baidu.com/f/search/res",
...
def headers_with_optional_cookie(base_headers):
...
def collect_post_links(session):
    """使用 requests 遍历搜索页，返回按发现顺序去重的完整帖子 URL。"""
    links = []
    seen = set()
    headers = headers_with_optional_cookie(SEARCH_HEADERS)
    for page in SEARCH_PAGE_RANGE:
        try:
            ...
            for link_tag in soup.find_all("a", class_="bluelink"):
                relative_link = link_tag.get("href", "").replace("&amp;", "&")
                if not relative_link.startswith("/p/"):
                    continue
                post_url = "https://tieba.baidu.com" + relative_link
...
def sync_browser_cookies(browser, session):
    for cookie in browser.get_cookies():
        session.cookies.set(cookie["name"], cookie["value"])


def download_images(browser, session, post_links):
    image_index = 1
    statistics = []
    for post_url in post_links:
        try:
            ...
            statistics.append([post_url, total_images, successful_images])
            time.sleep(random.uniform(5, 8))
        except Exception as exc:
            report_error(f"帖子处理异常，已跳过（{post_url}）", exc)
            statistics.append([post_url, 0, 0])
    return image_index - 1, statistics


def save_statistics(statistics):
    csv_path = os.path.join(SAVE_FOLDER, "帖子图片统计.csv")
    dataframe = pd.DataFrame(statistics, columns=["帖子链接", "总图片数", "成功下载数"])
    dataframe.to_csv(csv_path, index=True, index_label="序号", encoding="utf-8-sig")
    return csv_path
...
def main():
    ...
    print("开始采集搜索结果...")
    post_links = collect_post_links(session)
    print(f"第一阶段完成，共采集 {len(post_links)} 个帖子。")
    ...
    try:
        ...
        sync_browser_cookies(driver, session)

        downloaded_count, statistics = download_images(driver, session, post_links)
        csv_path = save_statistics(statistics)
        print("\n全部完成。")
        print(f"共下载图片：{downloaded_count} 张")
